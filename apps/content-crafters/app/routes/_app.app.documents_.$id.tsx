...
import { ClockIcon, DotsHorizontalIcon, DownloadIcon, Link1Icon, Share1Icon } from '@radix-ui/react-icons'
...
export async function loader({ params }: { params: { id: string } }) {
    const { error, data } = await supabase.from('documents').select('*').eq('id', params.id).single()
...
export async function action({ request, params }: { request: Request; params: { id: string } }) {
    ...
    const { data: item, error: itemError } = await supabase.from('documents').select('*').eq('id', id).single()
...
const DocumentPreview = () => {
    ...
    const handleCopyLink = () => {
        // copy to clipboard
        // @ts-ignore
      navigator.clipboard.writeText(`${window.ENV.APP_URL}/app/documents/${document.id}`)
                                ...
                                <DropdownMenu.Root>
                                    ...
                                    <DropdownMenu.Content>
                                        ...
                                        <DropdownMenu.Item
                                            shortcut="⌘ ⌫"
                                            color="red"
                                            onClick={() => setIsDeleting({ documentId: document.id })}
                                        >
                                            Delete
                                        </DropdownMenu.Item>
                                    </DropdownMenu.Content>
