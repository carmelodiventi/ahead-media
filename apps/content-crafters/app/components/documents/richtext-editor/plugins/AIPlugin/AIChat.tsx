...
const AIPlugin: React.FC<{ editor: LexicalEditor }> = ({ editor }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        aiAssistantChatRef.current &&
        !aiAssistantChatRef.current.contains(event.target as Node)
      ) {
  ...
  return (
    ...
    >
      <Chat
        editor={editor}
        ref={aiAssistantChatTextRef}
        onSubmit={handleAIAssistantResponse}
      />
    </Flex>
  );
};
